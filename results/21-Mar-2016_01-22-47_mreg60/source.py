...
def test_network(titoli = ['A2A','SRG','TRN','ENEL','EGPW'],indicatori=["mreg120","mreg60","vwapRatio","vwapRatioFTSE","minuto"],name="mreg60"):
    ...
    model = Sequential()
    model.add(Dense(output_dim=10, input_dim=len(indicatori), init="glorot_uniform")) #, weights=[np.array([[1],[1]]),np.array([0])])) #,  W_regularizer=l1(0.05))) # ,  W_regularizer=l1(0.05)))
