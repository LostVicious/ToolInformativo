...
def test_network(titoli = ['A2A','SRG','TRN','ENEL','EGPW'],name=""):
    ...
    model = Sequential()
    model.add(Dense(output_dim=64, input_dim=3, init="glorot_uniform")) #, weights=[np.array([[1],[1]]),np.array([0])])) #,  W_regularizer=l1(0.05))) # ,  W_regularizer=l1(0.05)))
    model.add(Activation("sigmoid"))
    model.add(Dropout(0.5))
    model.add(Dense(output_dim=64, input_dim=64, init="glorot_uniform")) #,  W_regularizer=l1(0.05))) # , 
    model.add(Activation("sigmoid"))
    model.add(Dropout(0.5))
    model.add(Dense(output_dim=1, input_dim=64, init="glorot_uniform")) #,  W_regularizer=l1(0.05)))
    model.add(Activation("sigmoid"))
