...
def test_network(titoli = ['A2A','SRG','TRN','ENEL','EGPW'],name="10neuroniSigmoid200epoc1"):
    ...
    model = Sequential()
    model.add(Dense(output_dim=10, input_dim=3, init="glorot_uniform")) #, weights=[np.array([[1],[1]]),np.array([0])])) #,  W_regularizer=l1(0.05))) # ,  W_regularizer=l1(0.05)))
    model.add(Activation("sigmoid"))
    #model.add(Dropout(0.1))
    model.add(Dense(output_dim=10, input_dim=10, init="glorot_uniform")) #,  W_regularizer=l1(0.05))) # , 
    model.add(Activation("sigmoid"))
    #model.add(Dropout(0.1))
    model.add(Dense(output_dim=1, input_dim=10, init="glorot_uniform")) #,  W_regularizer=l1(0.05)))
    model.add(Activation("sigmoid"))
