...
def generator():
    while 1:
        yield (np.array([1,2,3,4]),np.array([1,2,3,4]))
    
gain_field = "99_99"

def test_network(titoli = ['A2A','SRG','TRN','ENEL','EGPW'],name="20neuroniSigmoid200epocDROPOUT01"):
    ...
    for r in cursor:
        if (check_values(r)):
            #X_tot.append([(r["vwapRatio"]-1)*50,(r["vwapRatioFTSE"]-1)*50])
    ...
    model = Sequential()
    model.add(Dense(output_dim=20, input_dim=3, init="glorot_uniform")) #, weights=[np.array([[1],[1]]),np.array([0])])) #,  W_regularizer=l1(0.05))) # ,  W_regularizer=l1(0.05)))
    model.add(Activation("sigmoid"))
    model.add(Dropout(0.1))
    model.add(Dense(output_dim=20, input_dim=20, init="glorot_uniform")) #,  W_regularizer=l1(0.05))) # , 
    model.add(Activation("sigmoid"))
    model.add(Dropout(0.1))
    model.add(Dense(output_dim=1, input_dim=20, init="glorot_uniform")) #,  W_regularizer=l1(0.05)))
    model.add(Activation("sigmoid"))
    ...
    if (not load_model):
        ...
        model.fit_generator(generator, 1, s["n_epoch"],show_accuracy=True,callbacks=[LossHistory()])
    ...
    g = eval_model(model,X_train,Y_train,all_data_train,titoli)
    ...
    g = eval_model(model,X_validate,Y_validate,all_data_val,titoli)
