...
def check_values(r):
    #if (r["gainLong10days"] is None): return False
    if (r["pDaChiusura"] is None): return False
    ...
    if (r["ipercompratoEuro"] is None): return False
    if (r["ipercompratoTrades"] is None): return False
    ...
    if (r["regVolatility120"]) > 0.15: return False
    return True
...
def normalized_indicator(r,name):
    ...
    if name=='vwapRatio':
        ...
        return r[name]/5
    elif (name == 'regVolatility120'):
        return r[name]*10 #regVolatility va da 0 a 0.1 circa
    elif (name == 'bookImbalance30'):
        return (r["bookImbalance30"]-1)*1.0/2
    elif (name == 'ipercompratoEuro' or name == 'ipercompratoTrades'):
        return (r[name]-0.5)*2
    elif (name == 'pDaChiusura'):
        return (r['pDaChiusura']-1)*10
    else:
...
def custom_objective(y_true, y_pred):
    ...
    #long:
    y1 = T.nnet.sigmoid((y_pred-0.8)*1)
    a1 = T.sum(y_true*y1)
    #short:
    y2 = T.nnet.sigmoid((0.1-y_pred)*1) #dovrebbe darci 1 quando andiamo short
...
gain_field = "P1h" #"99_99"
X_train = None
Y_train = None
indicatori_usati = []

def test_network(
    titoli = ['TIT','FCA','UCG','ISP','ENEL','G','ENI'],
    #titoli = ['A2A','SRG','TRN','ENEL','EGPW'],
    #titoli = ['FNC','TEN','MB','UBI','BP'],        
    #titoli = ['IGD','BPSO','MT','CVAL','AMP','AST'],
    indicatori=["ipercompratoTrades","bookImbalance30","vwapRatio","vwapRatioFTSE","minuto"]#"mreg120","mreg60","regVolatility120","pDaChiusura","ipercompratoTrades","mreg120","mreg60","minuto"],
    NNN = 20,
    dropout=0.2,
    epoch=30,
    use_custom_objective=False, 
    activation='sigmoid',  #sigmoid
    name="20neuroniDropout02"):
    
    
    global indicatori_usati
    indicatori_usati = indicatori
    
    dirname = strftime("results/%d-%b-%Y_%H-%M-%S_", gmtime())+name+ '-'.join(titoli)+"_"+gain_field+"/"
    os.makedirs(dirname)
    ...
    for r in cursor:
        if (check_values(r)):
            ...
            if (gain_field[0] == "P"):
                ...
                r["gainShort"+gain_field] = - r["gainLong"+gain_field]
            else:
    ...
    sogliaShortMax = min(sogliaShortMax,0.2)
    
    print "soglia ottimale Long: "+ str(sogliaLongMax)
    print "soglia ottimale Short: "+ str(sogliaShortMax)
...
test_network() #titoli=['A2A','SRG','TRN','ENEL','EGPW','YNAP','MS','AGL','HER','CPR','FCA','LUX','MONC','SFER','TOD','BRE','AMP','REC'],name="tuttiTitoli25_22")
